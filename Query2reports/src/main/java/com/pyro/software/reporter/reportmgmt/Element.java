package com.pyro.software.reporter.reportmgmt;
...
import com.pyro.software.reporter.DashboardConstants;
import com.pyro.software.reporter.admin.connmgmt.ConnectionPool;
import com.pyro.software.reporter.utils.DWHUtility;
...
public class Element {
	...
	public void setElements(String zoneId,String circleId,String cityId) {
		this.zoneId=zoneId;
		this.circleId=circleId;
		this.cityId=cityId;
	}
	
	private String QueryFormation(String sql){
		
		if(this.zoneId.equalsIgnoreCase("1") && this.circleId.equalsIgnoreCase("0") && this.cityId.equalsIgnoreCase("0")){
			sql = sql.substring(0, sql.indexOf("$circle_start")).replace("$zoneId", zoneId)+")";
		}
		else if(this.zoneId.equalsIgnoreCase("1") && !this.circleId.equalsIgnoreCase("0") && this.cityId.equalsIgnoreCase("0")){
			sql = sql.substring(0, sql.indexOf("$city_start")).replace("$zoneId", zoneId).replace("$circleId", circleId).replace("circle_start", "")+")";
		}else{
			sql = sql.replace("$zoneId", zoneId).replace("$circleId", circleId).replace("$cityId", cityId).replace("circle_start", "").replace("city_start", "")+")";
		}
